...
ZOOM_VERSION_INSTALLED_MAJOR=$(dnf list installed | grep zoom | awk '{print $2}' | awk -F. '{print $1"."$2}')
ZOOM_VERSION_INSTALLED_MINOR=$(dnf list installed | grep zoom | awk '{print $2}' | awk -F. '{print $4}' | sed 's/-1//')
echo zoom version installed: $ZOOM_VERSION_INSTALLED
if [[ "$ZOOM_VERSION_INSTALLED_MINOR" != *"$ZOOM_VERSION_AVAILABLE_MINOR"* ]] || [[ "$ZOOM_VERSION_INSTALLED_MAJOR" != *"$ZOOM_VERSION_AVAILABLE_MAJOR"* ]]; then
